...
use application\service\HanahahahaService ;
class HanahahahaController extends  AppController
{
    /**
    * @var HanahahahaService
    */
    private $hanahahahaService;
    public function __construct($databaseConnection)
    {
        $this->setDbConn($databaseConnection);
        $this->hanahahahaService = new HanahahahaService($this->getDbConn());

    }
    public function __destruct()
    {
        $this->setDbConn(null);
        unset($this->hanahahahaService);
    }
    public function crudList()
    {
        ...
        $q_parameter = $this->initSearchParam(new Hanahahaha());

        $this->pushDataToView = $this->getDefaultResponse();
        $this->pushDataToView[SystemConstant::DATA_LIST_ATT] = $this->hanahahahaService->findAll($this->getRowPerPage(), $q_parameter);
        $this->pushDataToView[SystemConstant::APP_PAGINATION_ATT] = $this->hanahahahaService->getTotalPaging();
    ...
    public function crudAdd()
    {
        ...
        if(!empty($jsonData) && !empty($uid)) {
           $entity = new Hanahahaha($jsonData, $uid, false);
               $lastInsertId = $this->hanahahahaService->createByObject($entity);
               if ($lastInsertId) {
                    $this->pushDataToView = $this->setResponseStatus([SystemConstant::ENTITY_ATT => $this->hanahahahaService->findById($lastInsertId)], true, i18next::getTranslation(('success.insert_succesfull')));
                }
        }
        jsonResponse($this->pushDataToView);
    ...
    public function crudReadSingle()
    {
        ...
        $item = null;
        if ($id > 0) {
            $item = $this->hanahahahaService->findById($id);
            if ($item) {
    ...
    public function crudEdit()
    {
        ...
        if(!empty($jsonData) && !empty($uid)) {
           $hanahahaha = new Hanahahaha($jsonData, $uid, true);
                if (isset($hanahahaha->id)) {
                   $effectRow = $this->hanahahahaService->updateByObject($hanahahaha, array('id' => $hanahahaha->id));
                   if ($effectRow) {
    ...
    public function crudDelete()
    {
        ...
        if (count($idArray) > 0) {
            foreach ($idArray AS $id) {
                $entity = $this->hanahahahaService->findById($id);
                if ($entity) {
                    $effectRow = $this->hanahahahaService->deleteById($id);
                    if (!$effectRow) {
