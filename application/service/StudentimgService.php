...
use application\serviceInterface\StudentimgServiceInterface;
use application\model\Studentimg;
class StudentimgService extends BaseDatabaseSupport implements StudentimgServiceInterface
{
    protected $tableName = 'studentimg';
    ...
    public function findAll($perpage=0, $q_parameter=array())
    {
        ...
        $query .="FROM studentimg AS studentimg ";

		//default where query
        $query .=" WHERE studentimg.`id` IS NOT NULL ";
		//custom where query
       //$query .= "WHERE studentimg.custom_field =:customParam ";

        //gen additional query and sort order
       $additionalParam = $this->genAdditionalParamAndWhereForListPageV2($q_parameter, new Studentimg());
