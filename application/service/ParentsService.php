...
use application\serviceInterface\ParentsServiceInterface;
use application\model\Parents;
class ParentsService extends BaseDatabaseSupport implements ParentsServiceInterface
{
    ...
    public function findAll($perpage=0, $q_parameter=array())
    {
        ...
        $query .="FROM parents AS parents ";

		//default where query
        $query .=" WHERE parents.`id` IS NOT NULL ";
		//custom where query
       //$query .= "WHERE parents.custom_field =:customParam ";

        //gen additional query and sort order
       $additionalParam = $this->genAdditionalParamAndWhereForListPageV2($q_parameter, new Parents());
    ...
    public function findById($id)
    {
        ...
        $query .="FROM parents AS parents ";
        $query .="WHERE parents.`id`=:id ";

        $this->query($query);
        $this->bind(":id", (int)$id);
        return  $this->single();
    }
    public function findByStdCode($id)
    {
        $query = "SELECT *  ";

        $query .="FROM parents AS parents ";
        $query .="WHERE parents.`std_id`=:id ";
