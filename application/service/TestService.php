...
use application\serviceInterface\TestServiceInterface;
use application\model\Test;
class TestService extends BaseDatabaseSupport implements TestServiceInterface
{
    ...
    public function findAll($perpage=0, $q_parameter=array())
    {
		...
		//default where query
        $query .=" WHERE test.`id` IS NOT NULL ";
		//custom where query
       //$query .= "WHERE test.custom_field =:customParam ";

        //gen additional query and sort order
       $additionalParam = $this->genAdditionalParamAndWhereForListPageV2($q_parameter, new Test());
