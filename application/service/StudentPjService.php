...
use application\serviceInterface\StudentPjServiceInterface;
use application\model\StudentPj;
class StudentPjService extends BaseDatabaseSupport implements StudentPjServiceInterface
{
    protected $tableName = 'student_pj';
    ...
    public function findAll($perpage=0, $q_parameter=array())
    {
        ...
        $query .="FROM student_pj AS student_pj ";

		//default where query
        $query .=" WHERE student_pj.`id` IS NOT NULL ";
		//custom where query
       //$query .= "WHERE student_pj.custom_field =:customParam ";

        //gen additional query and sort order
       $additionalParam = $this->genAdditionalParamAndWhereForListPageV2($q_parameter, new StudentPj());
    ...
    public function findByCode($special)
    {
        $query = "SELECT * ";

        $query .="FROM student_pj ";
        $query .="WHERE special=:special ";
        $this->query($query);
        $this->bind(":special", $special);
        return $this->single();
