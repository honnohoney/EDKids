...
use application\serviceInterface\ImageServiceInterface;
use application\model\Image;
class ImageService extends BaseDatabaseSupport implements ImageServiceInterface
{
    ...
    public function findAll($perpage=0, $q_parameter=array())
    {
        ...
        $query .="FROM image AS image ";

		//default where query
        $query .=" WHERE image.`id` IS NOT NULL ";
		//custom where query
       //$query .= "WHERE image.custom_field =:customParam ";

        //gen additional query and sort order
       $additionalParam = $this->genAdditionalParamAndWhereForListPageV2($q_parameter, new Image());
