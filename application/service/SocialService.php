...
use application\serviceInterface\SocialServiceInterface;
use application\model\Social;
class SocialService extends BaseDatabaseSupport implements SocialServiceInterface
{
    ...
    public function findAll($perpage=0, $q_parameter=array())
    {
        ...
        $query .="FROM social AS social ";

		//default where query
        $query .=" WHERE social.`id` IS NOT NULL ";
		//custom where query
       //$query .= "WHERE social.custom_field =:customParam ";

        //gen additional query and sort order
       $additionalParam = $this->genAdditionalParamAndWhereForListPageV2($q_parameter, new Social());
    ...
    public function findbyTeacherId($id) {
        $query = "SELECT upload_time, msg, techer_id, image_id, first_name, last_name ";
        $query .= "FROM social INNER JOIN teacher ON social.techer_id = teacher.id ";
        $query .= "WHERE social.techer_id=:id";
